...
const StreamingDonghuaNavbar = ({ title, episodeTitle, episodeNumber, onBack }) => {
    return (
        <div className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/5 h-12 flex items-center px-3">
            ...
            {episodeNumber && (
                <span className="flex-shrink-0 ml-2 text-[10px] font-semibold px-2 py-0.5 bg-red-500/15 text-red-400 rounded-full">
                    EP {episodeNumber}
                </span>
            )}
        </div>
